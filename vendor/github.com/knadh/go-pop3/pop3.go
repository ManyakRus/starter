...
func (c *Conn) Cmd(cmd string, isMulti bool, args ...interface{}) (*bytes.Buffer, error) {
	...
	if len(args) > 0 {
		...
		// CMD arg1 argn ...\r\n
		cmdLine = cmd + fmt.Sprintf(format, args...)
	} else {
		cmdLine = cmd
	}
...
func (c *Conn) ReadAll() (*bytes.Buffer, error) {
	...
	for {
		...
		// Strip the trailing protocol line break.
		...
		// Remove the leading dot from lines starting with ".".
		// Spec "byte-stuffs" lines starting with "." by prefixing them with an additional dot
		// to prevent mis-interpretation of the line as the end of the multi-line response.
		// So, remove the leading dot.
...
func (c *Conn) Stat() (int, int, error) {
	...
	if len(f) < 2 {
		return 0, 0, fmt.Errorf("malformed STAT response")
	}
